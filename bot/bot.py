...
async def process_track_selection(callback: CallbackQuery):
    ...
    # Отправляем сообщение о начале загрузки с первым состоянием
    status_msg = await callback.message.answer(
        f"<b>🔍 [1/4] Поиск трека:</b>\n🎵 <b>{track['name']}</b>\n🎤 <b>{track['artist']}</b>\n\n<i>Ищу лучший источник для загрузки...</i>",
        parse_mode="HTML"
    )
    ...
    try:
        ...
        await asyncio.sleep(1.5)  # Небольшая задержка для лучшего UX
        await status_msg.edit_text(
            f"<b>⬇️ [2/4] Загрузка трека:</b>\n🎵 <b>{track['name']}</b>\n🎤 <b>{track['artist']}</b>\n\n<i>Загружаю аудиофайл, пожалуйста, подождите...</i>",
            parse_mode="HTML"
        )
        ...
        if file_path and os.path.exists(file_path):
            ...
            await status_msg.edit_text(
                f"<b>⚙️ [3/4] Обработка аудио:</b>\n🎵 <b>{track['name']}</b>\n🎤 <b>{track['artist']}</b>\n\n<i>Конвертирую и подготавливаю файл к отправке...</i>",
                parse_mode="HTML"
            )
            ...
            await status_msg.edit_text(
                f"<b>📤 [4/4] Отправка файла:</b>\n🎵 <b>{track['name']}</b>\n🎤 <b>{track['artist']}</b>\n\n<i>Загрузка завершена, отправляю файл...</i>",
                parse_mode="HTML"
            )
            ...
            # Удаляем сообщение о статусе загрузки вместо обновления статуса на "Готово"
